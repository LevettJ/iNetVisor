...
class Collect(object):
    ...
    def get_traceroute_measurements(self, start, duration, output, mapped=False):
        """
        ...
        while current_page >= 1:
            ...
            print("Processing: Page", current_page, end="", flush=True)

            count = 0
            if 'results' not in data:
                print("... no data.", end="", flush=True)
                break # Either failed to collect data, or there is none.
            
            print(" | Found", len(data['results']), "measurements | ", end="", flush=True)
            for i, measurement in enumerate(data['results']):
                ...
                try:
                    ...
                    try:
                        ...
                        print("| F:", measurement['result'], end="| ", flush=True)
                        continue

                if len(results) > 8000:
                    self.export(output + str(current_page) + "-" + str(i), mapped)
            ...
            self.export(output + str(current_page), mapped)
            print("S" + str(current_page), end=" ", flush=True)
    ...
    def _extract_paths(self, mapped=False):
        ...
        for traceroute in self.traceroutes:
            ...
            # Preconfigure traceroute format
            path = {
    ...
    def export(self, output, mapped=False):
        ...
        with open(output, 'w', encoding='utf-8') as f:
            ...
            f.flush()
        del self._export, self.traceroutes
        gc.collect()
        self._export = []
        self.traceroutes = []
...
if __name__ == '__main__':
    ...
    if args.router2as is not None:
        ...
        MAPPED = True
    traceObject.get_traceroute_measurements(args.start, args.duration, args.output, mapped=MAPPED)
